...
import granatamammolo.Java.granatamammolo.sort.*;
import java.io.File;
...
public class SortTiming {
  ...
  static public Timings measureAlgorithms(String[] algorithms, int minSize, int maxSize, int stepSize, ArrayBuilder builder) throws ReflectiveOperationException {
    ...
    for( int size=minSize; size<=maxSize; size+=stepSize ) {
      ...
      for( String algorithm : algorithms ) {
        log.info(algorithm);
        Method m = isort.class.getDeclaredMethod(algorithm, int[].class);
  ...
  public static void main(String[] args) throws ReflectiveOperationException, IOException {
    String[] quadratic = { "isort", "isortBin" , "isortBinMOD" };          // si può aggiungere: iSortBin
    
    ArrayBuilder builder = new ArrayBuilder(Integer.MIN_VALUE, Integer.MAX_VALUE);

    //ORIGINALI:
    writeResults("test.csv", measureAlgorithms(quadratic, 0, 5000, 100 , builder));
